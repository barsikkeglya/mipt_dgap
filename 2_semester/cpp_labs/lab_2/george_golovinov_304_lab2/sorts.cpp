...
void fill(int a[], int size, int m){
    //fills array with uniform distributed between 0 and maxvalue integers.
    std::uniform_int_distribution<> distr(0,m);
...
unsigned timing(int (*sort)(int a[], int start, int end), int a[], int begin_idx, int end_idx, int sample_size, bool changes, bool err){
    /*
    ...
    */
    ...
    //fill(a,maxlength,maxvalue);
    if (err) 
        return static_cast<unsigned>(floor((errors/sample_size)*100));
    if (changes)
        return static_cast<unsigned>(rolling_chg /= sample_size);
    return static_cast<unsigned>(time_span.count());
}

void run_auto(int (*sort)(int a[], int start, int end), int a[], int begin_idx, int end_idx, int runs, int sample_size, bool changes, bool errors){
    /*
    ...
    */
    ...
    for (int run = 0; run < runs; run++){
        std::cout << "run no " << run << "\n\n";
        for (int n = default_size; n <= maxlength;  n += 5000){
            std::cout << "(" << n << ","
            << timing(sort, a, 0, n-1, sample_size, changes, errors) << ")";
        }
        std::cout << "\n\n\n\n";
    }
}
...
int comb_sort(int a[], int begin_idx, int end_idx){
	...
	while (step >= 1){   
	    ...
	    step = static_cast<int>(round(static_cast<double>(step)/shrink_factor));
	}
...
int calculate_next_gap(int curr_gap, int mode){
    ...
    if (mode == 2){
        int n = curr_gap;
        int i = static_cast<int>(floor(std::log(n)/std::log(2)));
        return static_cast<int>(std::pow(2,i)-1);
    }
    if (mode == 3){
        double root5 = std::pow(5,0.5);
        double phi = (1 + root5)/2;
        int n = static_cast<int>(trunc((std::log((curr_gap-1)*root5))/std::log(phi)));
        return static_cast<int>(round(std::pow(phi,n)/root5));
    }
    return -1;
}
...
int main(){
    ...
    fill(a,maxlength,maxvalue);
    run_auto(shaker_sort,a,0,maxlength,amount_of_runs,sample_size, changes, errors);
    /*
    print_arr(a,100);
